...
public class MyBinaryHeap<E extends Comparable<? super E>> 
{
	...
	public E findKthLargest(int kth, E[] items)
	{
		heap = (E[]) new Comparable[kth+1]; // create a heap to store our largest vlues
		int p=0; // storing this value outside for loop to use it in the next loop
		for (; p<kth;p++)
		{
			insert(items[p]); // working correctly
			//System.out.println("System is adding "+items[p]+" to the heap");
		}
		buildHeap(); // Creates the heap in correct order, smallest to largest
		for (int i=p; i<items.length; i++)
		{	
			//System.out.println("System compares if "+findMin()+" is less than "+items[i]);
			if (findMin().compareTo(items[i]) < 0)
			{
				// System.out.println("System is adding "+items[i]+" to the heap");
				deleteMin(); // deletes root to make room for new value
				insert(items[i]); // inserts new value
			}
		}
		//System.out.println(this); // Prints all value in our LargestValues heap
		return findMin(); // Returns the root of our heap which is the kth largest value
	}
