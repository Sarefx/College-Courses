...
public final class BinomialQueue<AnyType extends Comparable<? super AnyType>>
{
    ...
    {
    	if (rhs.currentSize > currentSize)
    	{
    		System.out.println("Before the flip size: rhs is "+rhs.currentSize+" and lhs is "+currentSize);
    		BinomialQueue<AnyType> temp = new BinomialQueue<>();
    		temp.currentSize = this.currentSize;
    		temp.theTrees = this.theTrees;
    		this.currentSize = rhs.currentSize;
    		this.theTrees = rhs.theTrees;
    		rhs.currentSize = temp.currentSize;
    		rhs.theTrees = temp.theTrees;
    		System.out.println("After the flip size: rhs is "+rhs.currentSize+" and lhs is "+currentSize);
    	}
       	if( this == rhs )    // Avoid aliasing problems, this means lhs
            return;
        ...
        for( int i = 0, j = 1; j <= currentSize; i++, j *= 2 )
        {
            ...
            if (rhs.currentSize == 0 && carry == null)
            {
            	System.out.println("*****************Entered break statement*******************");
            	break;
            }
        }
    	...
    	System.out.println("Size of this after the merge is "+currentSize);
    }
    ...
    public static void main( String [ ] args )
    {
       /* int numItems = 10000;
        ...
        BinomialQueue<Integer> rhs = new BinomialQueue<>();
		BinomialQueue<Integer> lhs = new BinomialQueue<>();
		
		Random gen = new Random();
		
		int maxListSize = 5;
		int maxNumSize = 20;
		int lhsSize = gen.nextInt(maxListSize) + 1 ;
		//int lhsSize = 3;
		int rhsSize = gen.nextInt(maxListSize) + 1;
		//int rhsSize = 4;
		
		 for(int r = 0; r<lhsSize; r++ )
		 {
			 lhs.insert(gen.nextInt(maxNumSize)+1);
		 }
		 for(int r = 0; r<rhsSize; r++ )
		 {
			 rhs.insert(gen.nextInt(maxNumSize)+1);
		 }
		System.out.println("*BinomialQueue lhs min = "+lhs.findMin()+" and size = "+lhs.currentSize+"; rhs min = "+rhs.findMin()+" and size = "+rhs.currentSize);
		lhs.merge(rhs);
		System.out.println("*After lhs merge with rhs, min is "+lhs.findMin()+" and size is "+lhs.currentSize);
    }
}
